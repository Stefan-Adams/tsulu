...
{
	fprintf(stderr, "ERROR: %-15.15s %s\n", hdr, txt);
}

void debug_message(unsigned char *hdr, unsigned char *txt)
{
#ifdef _DEBUG
	fprintf(stderr, "DEBUG: %-15.15s %s\n", hdr, txt);
#endif
}
...
int Uproar_delete_track(int tracknum)
{
	...
	retval = uproar_send(dh, 0x0a, 0x00, 0x00, send_buffer, USB_BLOCK_SIZE);
	if ( retval < 0 ) {
	...
	memset(recv_buffer, 0, USB_BLOCK_SIZE);
	retval = uproar_recv(dh, 0x0c, 0x00, 0x00, recv_buffer, USB_BLOCK_SIZE);
	if ( retval < 0 ) {
	...
	send_buffer[4] = 0x46;
	send_buffer[5] = 0x2e;
	retval = uproar_send(dh, 0x0a, 0x01, 0x00, send_buffer, USB_BLOCK_SIZE);
	if ( retval < 0 ) {
		error_message("Uproar_delete_track", "uproar_send failed");
		uproar_close();
		return UPROAR_ERROR;
	...
	memset(recv_buffer, 0, USB_BLOCK_SIZE);
	retval = uproar_recv(dh, 0x0c, 0x01, 0x00, recv_buffer, USB_BLOCK_SIZE);
	if ( retval < 0 ) {
		error_message("Uproar_delete_track", "uproar_recv failed");
		uproar_close();
		return UPROAR_ERROR;
	...
	if ( (recv_buffer[4] != 0x00) || (recv_buffer[5] != 0x2e) ) {
...
int Uproar_reformat_memory()
{
	...
	if (Uproar_dev == NULL) {
		error_message("Uproar_reformat_memory", "need to initialize first");
		return UPROAR_ERROR;
	}
	...
	retval = uproar_send(dh, 0x0a, 0x00, 0x01, send_buffer, USB_BLOCK_SIZE);
	if (retval < 0) {
	...
	memset(recv_buffer, 0, USB_BLOCK_SIZE);
	retval = uproar_recv(dh, 0x0c, 0x00, 0x01, recv_buffer, USB_BLOCK_SIZE);
	if (retval < 0) {
	...
	memset(recv_buffer, 0, USB_BLOCK_SIZE);
	retval = uproar_recv(dh, 0x0c, 0x01, 0x01, recv_buffer, USB_BLOCK_SIZE);
	if ( retval < 0 ) {
		error_message("Uproar_reformat_memory", "uproar_recv failed");
		uproar_close();
		return UPROAR_ERROR;
	...
	if ( (recv_buffer[4] != 0x00) || (recv_buffer[5] != 0x2e) ) {
		error_message("Uproar_reformat_memory", "unexpected recv buffer 2");
		uproar_close();
		return UPROAR_ERROR;
	}
...
int Uproar_download(unsigned char *filepath)
{
	...
	while ( data_ptr < statbuf.st_size ) {
		fprintf(stderr, "%d/%d\r", data_ptr, statbuf.st_size);
#ifdef _DEBUG
		fprintf(stderr, "\nDEBUG: Uproar_download() data_ptr %d", data_ptr);
#endif
		memset(data_buffer, 0, SEND_TRACK_DATA_LEN);
		nread = fread(data_buffer, sizeof(unsigned char), SEND_TRACK_DATA_LEN, fp);
		data_ptr += nread;
		retval = usb_bulk_write(dh, USB_EP_WRITE, data_buffer, SEND_TRACK_DATA_LEN, USB_LONG_TIMEOUT);
		if ( retval < 0 ) {
	...
	fprintf(stderr, "%d/%d\r", statbuf.st_size, statbuf.st_size);
	fprintf(stderr, "\n");
